...
describe("Custom Tests", function () {
  ...
  it("[assignment] ii. deposit 0.1 ETH in L1 -> withdraw 0.08 ETH in L2 -> assert balances", async () => {
    ...
    expect(tornadoPoolBalance).to.be.equal(utils.parseEther("0.02")); // the difference
  });
  ...
  it("[assignment] iii. see assignment doc for details", async () => {
    ...
    // Alice sends 0.05 Edit: 0.06 ETH to Bob in L2
    ...
    const aliceWithdrawAmount = tornadoPoolBalance.toString(); // utils.parseEther('0.07')
    const aliceEthAddress = "0xdCad3a6d3569DF655070DEd06cb7A1b2Ccd1D3AF"; // aliceKeypair.address()
    const aliceWithdrawUtxo = new Utxo({
      amount: tornadoPoolBalance.sub(aliceWithdrawAmount),
      keypair: aliceDepositUtxo.keypair,
    });
    await transaction({
      tornadoPool,
